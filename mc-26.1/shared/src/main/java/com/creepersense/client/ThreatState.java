...
/**
 * Smoothed threat shown on the HUD (updated each tick in {@link CreeperSenseClient}).
 */
public final class ThreatState {
    ...
    // Small, allocation-free cache for smoothing multiple chevrons by entity id.
    ...
    public void setTargets(java.util.List<DetectedCreeper> creepers) {
        // Mark all existing as unseen by setting target intensity to 0; we'll re-fill from the list.
        for (int i = 0; i < count; i++) {
            targetPerIntensity[i] = 0f;
        }
        ...
        for (DetectedCreeper c : creepers) {
            ...
            if (idx < 0) {
                if (count >= MAX_TRACKED) {
                    // Replace the weakest (by display intensity) to keep the most relevant chevrons.
                    idx = indexOfWeakest();
                } else {
                    idx = count++;
                }
    ...
    public void smoothTowardTarget() {
        ...
        if (Math.abs(displayIntensity - targetIntensity) < 0.002f) {
            displayIntensity = targetIntensity;
        }

        // Per-creeper smoothing + compact removal
        int write = 0;
    ...
    /** Smoothed value used for drawing (0–1). */
